...
Passport strategies run with `session: false` (`08-backend-architecture.md` §4) — Passport's only job is the OAuth code-exchange handshake. Immediately after a successful callback, DevAtlas mints its own short-lived access JWT (15m default) and longer-lived refresh JWT (10d default), both set as `httpOnly`/`Secure`(prod)/`SameSite` cookies (`15-security-design.md` §4–§5). The refresh token is additionally persisted server-side as a sha256 hash on `User.refreshTokenHash`, enabling real revocation (logout, hash-mismatch detection) despite the access token itself being fully stateless. The frontend initially had no silent-reauth path at all: `apiSlice.js` had no 401-handling, so an expired access token just dropped the user to `/login` even with a fully valid refresh token available — closed by wrapping `baseQuery` to call `/auth/refresh` and retry on 401 (`09-frontend-architecture.md` §3.6), which is what makes the mechanism decided here actually deliver on "revocation without forcing re-login every 15 minutes" in practice, not just on paper.
...
- **Negative:** meaningfully more mechanism than "just use Passport sessions" — DevAtlas owns hash-based verification and a two-different-secrets discipline (access vs. refresh) itself, real surface area to get right. The mechanism actually evolved once already: an earlier version rotated the refresh token on every `/auth/refresh` call (classic reuse-detection posture), which surfaced a real bug — two requests racing the same access-token expiry (concurrent queries, or two browser tabs) could both rotate, and the loser's cookie would immediately fail as "revoked" well before its real ~10-day lifetime. `refreshAccessToken` no longer rotates on refresh (`15-security-design.md` §4) — `refreshTokenHash` changes only at an actual login or logout, which removes that race entirely at the cost of a stolen refresh token staying valid for its full remaining lifetime rather than being invalidated on the legitimate client's next use. Judged the right trade for a single-user-per-session app with no observed abuse case forcing the stronger posture; not re-litigated here, full reasoning lives in `15-security-design.md` §4.
- **Negative:** two cookies with two different lifetimes is inherently more moving parts than one session cookie. Accepted because the payoff — statelessness, fine-grained revocation, and prompt role propagation — matters specifically for a product where an admin promotion or deactivation needs to take effect immediately, not "eventually, once the session store catches up."
