...
*From `16-performance-design.md`:* route-level `lazy()` code-splitting for Card Detail and `/admin/*` (§4); the `{type:1, difficulty:1, status:1}` compound index, called out there as *required*, not optional (§7.3); the `sort` query-param allow-list guard (§3); `Attachment.width`/`height` plumbed through to fix layout-shift (§6); `compression` middleware (§9); `Cache-Control` headers on the genuinely public, non-personalized GET routes (§5.2). ~~The `baseQuery` reauth-on-401 wrapper~~ — done (`09-frontend-architecture.md` §3.6).

*From `09-frontend-architecture.md`:* `AppErrorBoundary` + `ErrorFallback` (§7 — doesn't exist at all today); the `AnimatePresence` route-transition wrapper (§5.1); `useReducedMotion()` on the two first-mount reveal animations (§5.3); `frontend/.prettierrc`, plus reformatting the remaining scaffold holdouts (`19-coding-standards.md` §9). ~~The standalone `CodeBlock` component~~ — done (§6.3), including the mermaid-fence special case and the `MarkdownField` inline-embedding toolbar it enables (§6.4) — neither was originally scoped for this milestone but both shipped together.

*Net-new hardening, not previously flagged elsewhere:* an accessibility pass — keyboard operability of highlight creation (`03-srs.md` NFR-4: "not mouse-drag-only"), focus management on route change, a color-contrast check against the oklch palette in both themes, `aria-live` regions for toasts/async state, a skip-to-content link; a representative load test (k6/Artillery) against the p95 budgets in `16-performance-design.md` §1, which `03-srs.md` §8.8 names as the actual production sign-off gate, not an optional nice-to-have. (The refresh-token race this item used to point at is closed differently than planned — see `15-security-design.md` §4: refresh no longer rotates the token at all, rather than rotating with a grace window.)
...
**Estimated Complexity.** L. Individually, most items are S/M-sized fixes — but there are roughly two dozen of them spanning three documents and both halves of the stack, and the load-test-driven items (index tuning) require iterating against real measurements rather than implementing a known fix once and moving on.
