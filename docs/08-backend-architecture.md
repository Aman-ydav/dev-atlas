...
| File | Responsibility |
|---|---|
| `utils/tokens.js` | Pure primitives: `generateAccessToken`, `generateRefreshToken`, `hashToken` (sha256), and `issueTokens(user)`, which signs both, persists `sha256(refreshToken)` onto `User.refreshTokenHash`, and returns the two raw tokens. Also exports `ACCESS_TOKEN_MAX_AGE_MS`/`REFRESH_TOKEN_MAX_AGE_MS` (parsed from the same `ACCESS_TOKEN_EXPIRY`/`REFRESH_TOKEN_EXPIRY` env values the JWTs themselves use) — `auth.controller.js`'s cookie options read these instead of a separately hardcoded millisecond literal, closing a real drift that existed early on (the refresh cookie's `maxAge` was hardcoded to 30 days while the token inside it actually expired in 10, so the cookie physically outlived its own contents by 20 days) |
...
The mechanics in one sentence: **the access token is stateless and short-lived, the refresh token is the only thing DevAtlas can actually revoke, and it revokes by hash comparison, not by a token blocklist.** The full rotation-on-use sequence and the reuse-detection response (why a hash mismatch nulls the whole session instead of failing quietly) are already fully specified in `15-security-design.md` §4 — this document doesn't re-derive that, it only names which file owns which part of the mechanism, above. The concurrent-refresh race that section flagged is now mitigated on the caller side, not the server side: `09-frontend-architecture.md` §3.6's `apiSlice.js` wrapper shares one in-flight `/auth/refresh` call across concurrent 401s instead of firing one per failed query, so this backend rotation logic in practice only ever sees one refresh attempt per actual expiry, not a race of several.
