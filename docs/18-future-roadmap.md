...
**Partial fix already shipped, upgrade still deferred.** Real usage surfaced exactly the "struggling cards not resurfacing often enough" symptom named above: the original leveled scheme scheduled every `forgot` a flat 1 day out regardless of how many times in a row a card was missed, so a just-failed card could never come back the same day. That's fixed within the *existing* leveled scheme — `forgot` now uses a short, level-independent relearning step (`REVISION_RELEARNING_MINUTES = 10`, `06-database-design.md` §5) instead of a day-granularity interval, and `shaky` picked up the same level-indexed ladder `confident` already had. This is a parameter/granularity fix inside the "deliberately not full SM-2" design, not the SM-2/FSRS upgrade itself — no `easinessFactor`/`stability`/`difficulty` state was added, the rating input is still three buttons, and everything in this section remains open.
